...
const infoTeam = async (page, league, query) => {
  ...
  for (let x = 0; x < league.table.length; x++) {
    ...
    const mainPageInfo = await page.evaluate(async () => {
      ...
      const name = document
        ...
        .trim();
      const img = document
        .querySelector('div[class="data-header__profile-ContainerOne"] img')
        .getAttribute("src");
  ...
  const pushToDb = async () => {
    await League_1.default.updateOne({ title: query }, { logos: logosData });
    for (let x = 0; x < data.length; x++) {
      ...
      const itsAlreadyInDb = await Team_1.default.find({
        "info.name": data[x].info.name,
      });
      if (itsAlreadyInDb.length === 0) {
        const newTeam = new Team_1.default(values);
        await newTeam.save();
      } else {
        await Team_1.default.updateOne(
          { "info.name": data[x].info.name },
          values
        );
      }
